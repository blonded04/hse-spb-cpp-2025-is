...
void send_message(tcp::socket &s, const std::string &msg) {
    assert(msg.size() < 0x10000);

    std::vector<unsigned char> len_bytes{
        static_cast<unsigned char>(msg.size() & 0xFF),
        static_cast<unsigned char>((msg.size() >> 8) & 0xFF)
    };
    write(s, buffer(len_bytes));
    write(s, buffer(msg));
}

std::string read_message(tcp::socket &s) {
    std::vector<unsigned char> len_bytes(2);
    read(s, buffer(len_bytes));

    int len = len_bytes[0] | (len_bytes[1] << 8);
