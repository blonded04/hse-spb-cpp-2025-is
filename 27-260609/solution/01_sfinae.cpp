...
template<typename T>
auto print(std::ostream &os, const T &container) -> decltype(os << *container.begin(), container.end(), void()) {  // not evaluated, no UB
...
template<typename T>
struct printer<T, std::void_t<decltype(std::declval<T>().print_to(std::declval<std::ostream&>()))>> {
    T value;
    void print_printable(std::ostream &os) {
        value.print_to(os);
    }
};
...
TEST_CASE("printer") {
    ...
    auto print_printable_is_valid = hana::is_valid([&](auto &v) -> decltype(v.print_printable(s)) {});
    ...
    SUBCASE("int") {
        ...
        CHECK(!print_printable_is_valid(p));
        CHECK(s.str() == "123");
    ...
    SUBCASE("printable") {
        ...
        CHECK(print_printable_is_valid(p));
        CHECK(s.str() == "foo");
        CHECK(std::is_same_v<void, decltype(p.print_printable(s))>);
    }
